          ...
          {canteen.images?.length > 0 && (
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {canteen.images.map((img, i) => <img key={i} src={img} alt={`img-${i}`} className="w-full h-24 object-cover rounded-xl cursor-pointer hover:opacity-90" onClick={() => setActiveDoc({ url: img, name: `Image ${i+1}` })} />)}
              </div>
            </div>
          )}
                ...
                <div key={c._id} className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700/60 p-5 hover:shadow-md transition-all flex flex-col gap-3">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-3">
                      {c.images?.[0] ? <img src={c.images[0]} alt={c.name} className="w-12 h-12 rounded-xl object-cover flex-shrink-0" />
