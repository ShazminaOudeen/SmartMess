...
const validate = (form) => {
  ...
  // Phone — 10 digits starting with 0, no letters or symbols
  if (!form.phone.trim())
    errors.phone = 'Phone number is required';
  else if (!/^\d+$/.test(form.phone.trim()))
    errors.phone = 'Phone number must contain digits only — no letters';
  else if (!/^0\d{9}$/.test(form.phone.trim()))
    errors.phone = 'Enter a valid 10-digit number starting with 0';
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md border border-gray-100 dark:border-gray-700 overflow-hidden max-h-[90vh] no-scroll-bar" style={{ overflowY: "auto", scrollbarWidth: "none", msOverflowStyle: "none" }}>
            ...
            <form onSubmit={handleAddAdmin} className="px-6 py-5 space-y-4">
              ...
              <InputField label="Email Address" name="email" type="email" value={form.email} onChange={handleChange} error={errors.email} placeholder="admin@smartmess.com" />
              ...
              <InputField label="Phone Number" name="phone" value={form.phone} onChange={handleChange} error={errors.phone} placeholder="0112345678" hint="10 digits starting with 0" maxLength={10} inputMode="numeric" />
