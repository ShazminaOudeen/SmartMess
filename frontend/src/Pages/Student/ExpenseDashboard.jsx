...
export default function ExpenseDashboard() {
  ...
  const generatePDF = () => {
    ...
    doc.text("Total Spent", 30, 66);
    doc.text("Total Orders", 95, 66);
    doc.text("Monthly Average", 155, 66);

    doc.setFontSize(12);
    doc.setFont("helvetica", "bold");
    doc.setTextColor(22, 163, 74);
    doc.text(`RM ${totalYear.toFixed(2)}`, 30, 76);
    doc.setTextColor(37, 99, 235);
    doc.text(`${totalOrders}`, 95, 76);
    doc.setTextColor(147, 51, 234);
    doc.text(`RM ${avgMonthly.toFixed(2)}`, 155, 76);
    ...
    autoTable(doc, {
      ...
      alternateRowStyles: {
        fillColor: [248, 255, 250],
      },
      columnStyles: {
        0: { fontStyle: "bold" },
        2: { textColor: [22, 163, 74], fontStyle: "bold" },
      },
      styles: {
        cellPadding: 4,
      ...
      foot: [[
        "TOTAL",
        totalOrders,
        `RM ${totalYear.toFixed(2)}`,
        `RM ${avgMonthly.toFixed(2)}`,
      ]],
      footStyles: {
              ...
              <thead>
                <tr className="border-b border-gray-100 dark:border-gray-700">
                  {["Month", "Orders", "Spent", "Avg / Order"].map((h) => (
                    <th key={h} className={`py-2.5 text-xs font-semibold text-gray-400 uppercase tracking-wider ${h === "Month" ? "text-left" : "text-right"}`}>
                      {h}
                    </th>
                  ))}
                </tr>
