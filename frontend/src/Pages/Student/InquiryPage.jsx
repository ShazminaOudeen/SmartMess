...
export default function InquiryPage() {
  ...
  const handleFile = (e) => {
    ...
    if (!file) return;
    if (file.size > 5 * 1024 * 1024) { setErrors((prev) => ({ ...prev, attachment: "File too large. Max 5MB." })); return; }
  ...
  return (
    <div className="min-h-screen px-6 py-8">
      <div className="max-w-2xl mx-auto">
        ...
        {/* Back */}
        <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-sm text-gray-500 hover:text-green-600 transition-colors mb-6 group">
        ...
        {showConfirm && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
            <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 mx-6 max-w-sm w-full shadow-2xl animate-scale-up">
              ...
              <p className="text-sm text-gray-500 mb-5">Our team will review and respond via {form.contactPref} within 24–48 hours.</p>
        ...
        <div className="card animate-fade-up space-y-5">
          ...
          <div className="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-4">
            <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-2">Your Info (Auto-filled)</p>
            <div className="flex items-center gap-3">
              ...
              <span className="ml-auto text-xs bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 px-2 py-0.5 rounded-full font-medium">Student</span>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {INQUIRY_TYPES.map((type) => (
                <button key={type} onClick={() => { handleChange("inquiryType", type); handleBlur("inquiryType"); }}
          ...
          <div>
            ...
            <input type="text" placeholder="Brief subject of your inquiry" value={form.subject}
          ...
          <div>
            ...
            <textarea placeholder="Please describe your inquiry in detail..." value={form.message} rows={5}
            ...
            <div className="flex items-center justify-between mt-1">
              ...
              <span className={`text-xs ml-auto ${form.message.length > 950 ? "text-red-500" : form.message.length >= 20 ? "text-green-500" : "text-gray-400"}`}>
          ...
          <div>
            ...
            <div className="flex gap-3">
              {CONTACT_PREFS.map((pref) => (
                <button key={pref} onClick={() => handleChange("contactPref", pref)}
          ...
          {Object.keys(errors).filter(k => k !== "submit" && k !== "attachment" && touched[k]).length > 0 && (
            <div className="bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-800 rounded-xl px-4 py-3">
              ...
              <ul className="space-y-0.5">
                {Object.entries(errors).filter(([k]) => k !== "submit" && k !== "attachment" && touched[k]).map(([k, v]) => (
