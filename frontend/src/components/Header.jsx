      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-4">
            ...
            >
              <div
                className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow-md transition-all duration-300 flex items-center justify-center text-[8px] ${dark ? "left-[22px]" : "left-0.5"
                  }`}
              >
                {dark ? "🌙" : "☀️"}
              </div>
          ...
          >
            <span
              className={`w-6 h-0.5 bg-gray-800 dark:bg-white transition-transform duration-300 ${menuOpen ? "rotate-45 translate-y-1.5" : ""
                }`}
            />
            <span
              className={`w-6 h-0.5 bg-gray-800 dark:bg-white transition-opacity duration-300 ${menuOpen ? "opacity-0" : ""
                }`}
            />
            <span
              className={`w-6 h-0.5 bg-gray-800 dark:bg-white transition-transform duration-300 ${menuOpen ? "-rotate-45 -translate-y-1.5" : ""
      ...
      {menuOpen && (
        <div className="md:hidden bg-white/80 dark:bg-gray-900/80 backdrop-blur-lg border-b border-gray-200/50 dark:border-gray-800/50">
          <div className="px-4 py-4 space-y-2">
            {navLinks.map((link) => {
              ...
              return link === "Browse Canteens" ? (
                <Link
                  key={link}
                  to="/canteens"
                  className={`block px-4 py-2 rounded-lg text-gray-600 dark:text-gray-300 ${isActive
            ...
            <div className="flex gap-3 pt-4 mt-2 border-t border-gray-200/50 dark:border-gray-800/50">
              {isAuthenticated ? (
                <>
                  <Link
                    to="/profile"
                ...
                <>
                  ...
                  <Link
                    to="/register/student"
                    className="flex-1 px-4 py-2 text-center rounded-lg text-sm font-semibold text-white bg-gradient-to-r from-primary-500 to-primary-600 hover:from-primary-600 hover:to-primary-700 shadow-md shadow-primary-500/30 transition-all duration-200 backdrop-blur-sm"
                    onClick={() => setMenuOpen(false)}
                  >
                    Register
                  </Link>
