...
const transporter = nodemailer.createTransport({
  service: 'gmail',
  auth: {
    user: 'shazminalap@gmail.com',
    pass: 'pebj otqn vihb fvlr',
  },
  tls: {
...
const sendEmail = async (to, subject, html) => {
  ...
  from: '"SmartMess Canteen" <shazminalap@gmail.com>',
  to, 
  subject, 
  html 
});
...
const acceptedEmailHtml = (order) => `
  <div style="font-family:Arial,sans-serif;max-width:500px;margin:auto;border:1px solid #e5e7eb;border-radius:12px;overflow:hidden">
    ...
    <div style="padding:24px">
      ...
      <p style="color:#374151">Your order <strong>#${order._id.toString().slice(-6).toUpperCase()}</strong> has been <strong style="color:#16a34a">accepted</strong> by the canteen and is being prepared.</p>
...
const rejectedEmailHtml = (order) => `
  <div style="font-family:Arial,sans-serif;max-width:500px;margin:auto;border:1px solid #e5e7eb;border-radius:12px;overflow:hidden">
    ...
    <div style="padding:24px">
      <p style="color:#374151">Hi <strong>${order.studentName}</strong>,</p>
      <p style="color:#374151">Unfortunately your order <strong>#${order._id.toString().slice(-6).toUpperCase()}</strong> has been <strong style="color:#ef4444">rejected</strong> by the canteen.</p>
      ...
      <p style="color:#6b7280;font-size:14px">We apologize for the inconvenience. Please try ordering again or contact the canteen for more information.</p>
    </div>
    <div style="background:#f9fafb;padding:16px;text-align:center">
      <p style="color:#9ca3af;font-size:12px;margin:0">SmartMess — Canteen Management System</p>
    </div>
...
const acceptOrder = async (req, res) => {
  try {
    ...
    if (order.studentEmail) {
      await sendEmail(
        order.studentEmail,
        `Order Accepted — #${order._id.toString().slice(-6).toUpperCase()}`,
        acceptedEmailHtml(order)
      );
    }
...
const rejectOrder = async (req, res) => {
  try {
    ...
    if (order.studentEmail) {
      await sendEmail(
        order.studentEmail,
        `Order Rejected — #${order._id.toString().slice(-6).toUpperCase()}`,
        rejectedEmailHtml(order)
      );
    }
